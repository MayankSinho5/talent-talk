...
# --- Page Config & Styling ---
st.set_page_config(page_title="Talent Talk - Voice Interviewer", layout="wide", page_icon="🎙️")
...
st.markdown('<p class="subtitle">AI-powered technical interviews with local voice interaction (Whisper)</p>', unsafe_allow_html=True)
...
if input_method == "Voice":
    ...
    language = st.sidebar.selectbox("Speech Language", ["en", "auto"], index=0)
    ...
    if input_method == "Voice":
        ...
        if audio_bytes:
            ...
            if duration > MAX_RECORD_SECONDS:
                ...
                with st.status("Transcribing audio locally...", expanded=True) as status:
